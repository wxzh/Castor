...
//      ucid ^^ { tn => ctx: Context => if (ctx.isNameBound(tn)) TyVar(ctx.name2index(tn), ctx.length) else TyId(tn) } |
  ...
  lazy val appTm: PackratParser[Res[Tm]] =
    appTm ~ pathTm ^^ { case t1 ~ t2 => ctx: Context => TmApp(t1(ctx), t2(ctx)) } |
      "fix" ~> pathTm ^^ { t => ctx: Context => TmFix(t(ctx)) } |
      "succ" ~> pathTm ^^ { t => ctx: Context => TmSucc(t(ctx)) } |
      "pred" ~> pathTm ^^ { t => ctx: Context => TmPred(t(ctx)) } |
      "iszero" ~> pathTm ^^ { t => ctx: Context => TmIsZero(t(ctx)) } |
      pathTm

  lazy val ascribeTm: PackratParser[Res[Tm]] =
    aTm ~ ("as" ~> `type`) ^^ { case t ~ ty => ctx: Context => TmAscribe(t(ctx), ty(ctx)) } |
      aTm
  ...
  lazy val termSeq: PackratParser[Res[Tm]] =
